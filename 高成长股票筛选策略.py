...
def get_market_cap(security, date):
    """获取市值"""
    try:
        # 在Ptrade平台上使用get_fundamentals获取市值
...
def get_total_shares(security, date):
    """获取总股本"""
    try:
        # 在Ptrade平台上使用get_fundamentals获取总股本
        from jqdata import query, valuation
        date_str = date.strftime('%Y%m%d')
...
def get_gross_margin(security, date):
    """获取毛利率"""
    try:
        # 在Ptrade平台上使用get_fundamentals从profit_ability表获取毛利率
...
def get_rd_ratio(security, date):
    """获取研发投入占营收比例"""
    try:
        # 在Ptrade平台上使用get_fundamentals获取研发投入和营收
...
def get_revenue_growth(security, date):
    """获取营收增速"""
    try:
        # 在Ptrade平台上使用get_fundamentals获取营收增速
...
def get_profit_growth(security, date):
    """获取归母净利润增速"""
    try:
        # 在Ptrade平台上使用get_fundamentals获取净利润增速
        from jqdata import query, income
        date_str = date.strftime('%Y%m%d')
