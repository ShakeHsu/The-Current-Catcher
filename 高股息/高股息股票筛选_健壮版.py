...
def get_real_time_data():
    ...
    try:
        df = retry_request(fetch_data, max_retries=3, delay=2)
    ...
    def fetch_data():
        ...
        if finance_df is not None and not finance_df.empty:
            ...
            return {
                ...
                'payout_ratio': safe_float(latest.get('股利支付率', 0)),
                'pe_ttm': safe_float(latest.get('市盈率', 0)),
                'eps': safe_float(latest.get('基本每股收益', 0))
            }
        return {'profit_growth': 0, 'payout_ratio': 0, 'pe_ttm': 0, 'eps': 0}
    
    try:
        return retry_request(fetch_data, max_retries=2, delay=1)
    except Exception as e:
...
def get_dividend_history(stock_code):
    ...
    def fetch_data():
        ...
        if dividend_df is not None and not dividend_df.empty:
            ...
            dividend = safe_float(latest.get('每股派息', 0))
            return dividend
        return 0
...
def process_stocks(stocks, max_stocks=50):
    ...
    for i, stock in enumerate(stocks[:max_stocks]):
        if (i + 1) % 10 == 0:
            print(f"处理进度: {i + 1}/{count} (成功: {success_count}, 失败: {fail_count})")
